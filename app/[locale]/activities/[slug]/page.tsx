...
const categoryTkeys: Record<string, string> = {
  'self-guided': 'selfGuided', 'canal-tour': 'canalTour',
  'watersport': 'watersport', 'private': 'private', 'unique': 'unique',
}
    ...
    <div className="min-h-screen bg-[#faf7f2] pt-16">
      ...
      <div className="bg-[#0a3d52] pt-12 pb-0">
        <div className="max-w-5xl mx-auto px-5">
          ...
          {locale !== 'en' && (
            <div className="bg-white/10 border border-white/15 rounded-xl px-4 py-2.5 mb-6 text-xs text-white/75">
              🌐 {tc('contentEnNotice')}
            </div>
          )}
            ...
            <div className="flex-1">
              <div className="flex flex-wrap gap-2 mb-3">
                <span className="text-xs font-semibold bg-white/10 text-white/80 px-3 py-1 rounded-full">
                  {tCat(categoryTkeys[activity.category])}
                </span>
                {activity.popular && (
                  <span className="text-xs font-semibold bg-amber text-white px-3 py-1 rounded-full">{tAct('popular')}</span>
                )}
                {activity.isNew && (
                  <span className="text-xs font-semibold bg-canal text-white px-3 py-1 rounded-full">{tAct('new')}</span>
                )}
              </div>
      ...
      <div className="max-w-5xl mx-auto px-5 py-10">
          ...
          <div className="md:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                ['⭐', t('rating'),    `${activity.rating} (${activity.reviewCount} ${t('reviewsSuffix')})`],
            ...
            {/* Description */}
            <div>
              <h2 className="font-display font-bold text-canal-dark text-xl mb-3">{t('aboutTitle')}</h2>
            ...
            {activity.highlights?.length > 0 && (
              <div>
                <h2 className="font-display font-bold text-canal-dark text-xl mb-3">{t('highlights')}</h2>
            ...
            {activity.included?.length > 0 && (
              <div>
                <h2 className="font-display font-bold text-canal-dark text-xl mb-3">{t('included')}</h2>
                <ul className="space-y-2">
            ...
            {activity.reviewQuote && (
              <div className="bg-canal-light rounded-2xl p-6 border border-blue-100">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-slate-400 text-sm">· {activity.reviewCount.toLocaleString()} {t('reviewsSuffix')}</span>
                </div>
          ...
          <div className="md:col-span-1">
            <div className="sticky top-20 bg-white rounded-2xl border border-stone-200 shadow-lg p-6">
              ...
              <p className="text-xs text-slate-400 text-center mt-3">
                {t('bookingNotice', { provider: activity.provider })}
              </p>
            </div>
          </div>
        </div>
        ...
        {related.length > 0 && (
          <div className="mt-14">
            <h2 className="font-display font-bold text-canal-dark text-2xl mb-6">{t('similarTitle')}</h2>
